/**
 * Admin · double-click no Exportar CSV enquanto aria-busy="true".
 *
 * Contrato: enquanto o botão está com aria-busy=true (exporting), um
 * segundo clique não deve gerar novo download nem nova requisição HTTP.
 */
import { test, expect } from "@playwright/test";
import { ensureSignedIn } from "./_helpers/auth";

test.describe("Admin · Exportar CSV — clique durante aria-busy", () => {
  test("segundo clique com aria-busy=true não dispara request nem download extra", async ({
    page,
  }) => {
    ...
    // Espera aria-busy=true (estado exporting confirmado).
    await expect(btn).toHaveAttribute("aria-busy", "true");
    await expect(btn).toHaveAttribute("data-exporting", "true");

    // 2º clique enquanto aria-busy=true — deve ser ignorado (disabled).
    await btn.click({ force: true }).catch(() => {});
    await btn.click({ force: true }).catch(() => {});

    // Aguarda o handler terminar.
