    ...
    handlers: {
      GET: async () => {
        ...
        const xml = [
          ...
          `    /auth, /reset-password, /checkout/return  — auth/payment flows tagged noindex,nofollow`,
          `    /app/*                                    — authenticated app, login-walled and tagged noindex,nofollow at layout level`,
          `    /api/*                                    — JSON/API endpoints blocked in robots.txt, not HTML pages`,
