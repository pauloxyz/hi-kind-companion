...
function AdminStripeEventsPage() {
  ...
  async function handleExport(format: "csv" | "json") {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/stripe-events-export?format=${format}`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
        },
        body: JSON.stringify({ ...filterPayload, sortBy, sortDir }),
      });
      if (!res.ok) {
      ...
      const filename =
        ...
        buildStripeEventsFilenameFallback(
          { environment, status, eventType },
          format,
        );
        ...
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Button
              size="sm"
              onClick={() => handleExport("csv")}
              disabled={exporting !== null || total === 0 || batchPending}
